...
bool process_keys(void (*key_press_callback)(int), void (*key_release_callback)(int))
{
	...
	for (int i = 0; i < NR_KEYS; i++)
	{
		...
		//volatile int state0 = query_key(i);
		//volatile int state1 = query_key(i);
		//if (/*state0 == KEY_STATE_EMPTY &&*/ state1 == KEY_STATE_PRESS) {
		//	key_press_callback(get_keycode(i));
		//	ret = true;
		//	break;
		//}
		//if (/*state0 == KEY_STATE_PRESS &&*/ state1 == KEY_STATE_WAIT_RELEASE) {
