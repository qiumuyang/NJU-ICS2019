...
#define make_instr_impl_1op_jcc(inst_name, src_type, suffix, cc) \
	...
	{                                                           \
		...
		OPERAND prt; \
	    prt.type = OPR_PRT; \
	    prt.val = eip + sign_ext(opr_src.val, opr_src.data_size) + len; \
	    print_asm_1(#inst_name, #cc, len, &prt); \
...
#define condition_g \
	(cpu.eflags.OF == cpu.eflags.SF) && !cpu.eflags.ZF
